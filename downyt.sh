...
if [ $SEARCH == 2 ]; then
	...
	yt-dlp --sub-lang en --write-auto-sub --write-sub --embed-sub -f '22/worst' --external-downloader aria2c --external-downloader-args '-c -j 3 -x 3 -s 3 -k 1M' "ytsearch:$TITLE"
    ...
    #used mostly for Twitter videos where the name is too long hence no option for video quality
		yt-dlp -f '22/worst' --external-downloader aria2c --external-downloader-args '-c -j 3 -x 3 -s 3 -k 1M' --id $URL
